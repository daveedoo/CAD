...
namespace GL
{
	...
	std::string Shader::ReadFileContents(std::filesystem::path shaderSourcePath)
	{
		...
		if (!shaderSourceFile)
		{
			std::string excMsg = (boost::format("Shader::ReadFileContents - could not open shader source file: \"%1%\"") % shaderSourcePath).str();
		...
		if (!shaderSourceFile)
		{
			std::string excMsg = (boost::format("Shader::ReadFileContents - could not read shader source file: \"%1%\"") % shaderSourcePath).str();
			throw std::exception(excMsg.c_str());
		}
	...
	GLuint Shader::CompileShader(std::string shaderSourceCode)
	{
		...
		if (compilationStatus == GL_FALSE)
		{
			...
			std::string excMsg = (boost::format("Shader::CompileShader - could not compile shader. Compilation info:\n%1%") % compilationInfo).str();
			throw std::exception(excMsg.c_str());
		}
		return shaderID;
	}
